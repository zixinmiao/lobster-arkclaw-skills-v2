...
- `member_vs_nonmember_split`：会员/非会员试衣占比
- `purchase_signal_distribution`（v2.1）：基于 `not_buy_reason` 与 `followup_intent` 推导的成交/待考虑/未成交分布（不依赖单独的 try_on_result 字段）
...
- `raw_notes` 为空（数据完整性异常，所有反馈字段都没法写出来时也算）
...
- 商品件数
- 会员试衣单数
- 成交意向分布（基于 not_buy_reason / followup_intent 派生：高意向 / 待考虑 / 未成交）
- 品类分布
- 异常记录数
- 表现亮点
- 待补录提醒
