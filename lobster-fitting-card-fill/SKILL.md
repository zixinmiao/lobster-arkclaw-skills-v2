...
### 基础区
- `session_id`（不可编辑）
- `guide_name`（必填；缺失时展示空输入，不得预填 `导购` / `店员` / `未知` 等占位词）
- `store_name`
...
- 卡片提交结果应对齐 v2.1 主表保留字段（20 字段集），不再要求已删除的字段（`source_bundle_id` / `source_message_ids` / `is_member` / `try_on_result` 等）
- 若 `guide_name` 为空或仅为角色名/占位词，提交必须被阻断，并提示"请补充导购姓名，之后同一导购可自动沿用"
- 卡片提交后的 `guide_name` 来源标记为 `card_confirmed`，后续可写入 `sender_profile` 供同一 sender 继承
