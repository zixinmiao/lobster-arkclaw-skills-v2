...
| `enum_violations` | 数组；强枚举字段值不在合法集合中（如 try_on_result 写了自由文本） |
| `validation_summary` | 字符串；可读摘要，用于上游打日志或发卡片 |

## 各表 required 集合（来自 contract）

### 试衣商品记录（v2，8 个 required）
- `session_id`
- `guide_name`
- `fitting_at`
- `operator_id`
...
```json
{
  "result": {
    ...
    "enum_violations": [
      {"field": "try_on_result", "value": "可能买", "allowed": ["合适", "待考虑", "未成交", "已购"]}
    ],
